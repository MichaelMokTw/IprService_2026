...
namespace IprService.Ipr {
    public partial class IprWorker {
        ...
        public void ProcessData_H323(SsmEventDataModel ssmEvt) {
            uint slaverID = 0;
            uint DTMF = 0;
            uint ret = 0;
            string errMsg = "";
            ...
            _chLog!.Info($"[{_chType}][{eventName,-32}], nRef={nRef}, staID={stationID}, hWChID={_hwChID}, ursChID={_ursChID}, extNo={_extNo}, callRef={ssmEvt.CallInfo.CallRef} RawData=[{ssmEvt.RawData}]");
            switch (ssmEvt.wEventCode) {
                ...
                case (ushort)EventCode.E_PROC_RecordEnd:
                    ...
                    // 暫時不要
                    //if (ssmEvt.dwParam == 7) { // stopRecByRTPTimeout
                    //    _chLog!.Info($"\t -> stopRecByRTPTimeout => try get ANA IPR hwChID={_iprAnaID} ...");
                    //    var ipr = GVar.IprManager.GetIpr(_iprAnaID);
                    //    if (ipr != null) {
                    //        _chLog!.Info($"\t -> got ANAIPR, call  StopRecording ...");
                    //        ipr.StopRecording(ssmEvt.CallInfo.CallRef);
                    //    }
                    //    else {
                    //        _chLog!.Error($"\t -> failed to get ANA IPR，please check system.");
                    //    }
                    //}
                    break;
                ...
                case (ushort)EventCode.E_RCV_IPR_DChannel:
                    ProcessDChannelEvent_H323(ssmEvt); // <= 統一處理有關 D-Channel 的 Event
                    break;
                ...
                case (ushort)EventCode.E_RCV_IPR_AUX_MEDIA_SESSION_STOPED:
                    ...
                    // 以上是 SIP，則停止錄音
                    //if (iprCh.CallState == (int)ENUM_CallState.Idle && iprCh.RecordingState != (int)ENUM_RecordingState.Idle) { // <== SIP，停止錄音前 CallState 會改變，但 H323 不會
                    if (iprCh.RecordingState != (int)ENUM_RecordingState.Idle) {
        ...
        private void ProcessDChannelEvent_H323(SsmEventDataModel ssmEvt) {
            var stationID = ssmEvt.stationID;
