...
namespace MyProject.lib {
    public static class lib_misc {
        ...
        public static int CopyFile(string from, string to, out string err) {
            ...
            try
            {
                global::System.IO.File.Copy(from, to, true);
            }
            catch (Exception ex)
            {
                err = ex.Message;
        ...
        public static bool RenameFile(string from, string to, out string err) {
            var ret = true;
            err = "";
            try {
                File.Move(from, to);
        ...
        public static decimal GetFileSizeMB(string fileName) {
            ...
            if (File.Exists(fileName)) {
                try {
                    var fileInfo = new global::System.IO.FileInfo(fileName);
        ...
        public static long GetFileByte(string fileName) {
            long size = 0;
            if (File.Exists(fileName)) {
                try {
